      ...
      <div className="max-w-5xl w-full relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {taals.map((taal) => (
            <div 
              key={taal.name} 
              className="group bg-[#241712]/80 backdrop-blur-md border border-classical-wood/60 rounded-3xl p-8 hover:border-classical-gold/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_10px_40px_-10px_rgba(229,169,55,0.15)] flex flex-col h-full relative overflow-hidden"
            >
              {/* Subtle top glow line */}
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-classical-gold/20 to-transparent group-hover:via-classical-gold transition-all duration-500"></div>
        ...
        <div className="mt-20 text-center">
          <Link 
            href="/classify" 
            className="inline-block px-10 py-4 border border-classical-wood text-classical-gold font-serif font-bold tracking-widest rounded-full hover:bg-classical-wood hover:text-white transition-all duration-300 hover:shadow-[0_0_20px_rgba(229,169,55,0.2)]"
          >
            RETURN TO ANALYSIS
          </Link>
        </div>
